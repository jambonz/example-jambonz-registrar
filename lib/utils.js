...
module.exports = (logger) => {
  ...
  /* populate user credentials for rapid access */
  if (userDb.size === 0) {
    for (const [domainName, domainInfo] of Object.entries(credentials)) {
      for (const [userName, userInfo] of Object.entries(domainInfo)) {
        const aor = `${userName}@${domainName}`;
        userDb.set(aor, userInfo);
      }
    }
    logger.info(`populated in-memory database of ${userDb.size} users`);
  }

  const getUserInfo = (user, domain) => {
    const aor = domain ? `${user}@${domain}` : user;
    return userDb.get(aor);
  };
